...
# Open issues at: https://github.com/authorken
...
import sys
init(autoreset=True)
__version__ = "Author: ken"
__github__= "https://github.com/authorken"
dir_path = os.path.dirname(os.path.realpath(__file__))
configur = ConfigParser()
configur.read(os.path.join(dir_path, f"setup.ini"))
...
def sys_c_t():
   if configur.get("sys","TOKEN") != "":
      pass
        ...
        print(f"{Lb}[!]{Fore.RED} No token found. You must paste your token inside the 'setup.ini' file, in front of the value 'TOKEN'.")
        exit()
       ...
       print(f"{Lb}[!]{Fore.RED} Invalid config detected. Please re-check the config file, `setup.ini` and your settings.")
       exit()
available_usernames = []
...
def setdelay():
   global Delay
   print(f"{Lb}[!]{Ly} Default delay is: {Delay}s (setup.ini){Lb}")
...
def proc0():
    m_input = input(f"{Fore.LIGHTBLACK_EX}[{Fore.LIGHTGREEN_EX}ken{Fore.LIGHTBLACK_EX}]:> {Fore.LIGHTYELLOW_EX}").lower()
    if m_input=="exit":
        sys.exit(0)
    if m_input=="":
        proc0()
   ...
   if opt == 2:
    for username in usernames:
       ...
       if json_endpoint.get("taken") is not None:
           ...
           print(f"{Lb}[?]{Fore.RED} Error validating '{username}': {endpoint.json()['message']} |ken: Make sure you have a valid token.")
   elif opt == 1:
       body = {
           "username": usernames
       }
       ...
       if json_endpoint.get("taken") is not None:
           ...
           print(f"{Lb}[?]{Fore.RED} Error validating '{usernames}': {endpoint.json()['message']} |ken: Make sure you have a valid token.")
...
def ch_send_webhook(val0:str):
   if webhook_0 == True:
       ...
       embeds=[
    {
      ...
      "author": {
        "name": "ken",
        "url": "https://github.com/authorken",
       ...
       print(f"{Lb}[!]{Fore.RED} Error: Something went wrong while sending the webhook request. Exception: {s} | ken: Make sure you have a valid webhook URL")
   else:
      return
def opt1func(v1,v2):
   for i in range(v1):
